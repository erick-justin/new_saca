...
bool validateInputs(BuildContext context, List<String> inputs) {
  ...
  for (int i = 0; i < inputs.length; i++) {
    ...
    if (input.isEmpty) {
      MessageDialogs()
          .messageDialog(context, "Alert", "Please Fill All The Fields", 3464);
      valid = false;
      break;
    }
  }
...
bool validatePhoneNumber(BuildContext context, String number) {
  ...
  if (number.length != 10 && number.length != 12) {
    ...
    MessageDialogs()
        .messageDialog(context, "Alert", "Enter A valid Phone number", 3464);
    valid = false;
  }
  return valid;
}

void showModalBottomSheetF(BuildContext context, Widget body,
...
String getCommaValueStrict(double number) {
  var ss = number.toStringAsFixed(0);
  RegExp reg = new RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))');
  Function mathFunc = (Match match) => '${match[1]},';
  ss = ss.replaceAllMapped(reg, (Match match) => '${match[1]},');
